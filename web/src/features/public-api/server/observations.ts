...
type CursorQueryType = Omit<QueryType, "page"> & {
  cursor?: {
...
const buildObservationFilters = (props: QueryType): Prisma.Sql[] => {
...
const buildTraceFilters = (props: QueryType): Prisma.Sql[] => {
  const clauses: Prisma.Sql[] = [];
...
export const generateObservationsV2ForPublicApi = async ({
  props,
  advancedFilters,
}: {
  props: CursorQueryType;
  advancedFilters?: FilterState;
}): Promise<Observation[]> => {
  if (advancedFilters?.length) {
    logger.warn(
  ...
  const whereObservationSql = Prisma.join(
    buildObservationFilters({ ...props, page: 1 }),
    " AND ",
  );
  const traceFilters = buildTraceFilters({ ...props, page: 1 });
  const traceFilterSql =
    traceFilters.length > 0
      ? Prisma.sql`AND ${Prisma.join(traceFilters, " AND ")}`
      : Prisma.empty;
  const cursorWhereSql = props.cursor
    ? Prisma.sql`
      WHERE (
        lo.start_time < ${props.cursor.lastStartTimeTo}
        OR (
          lo.start_time = ${props.cursor.lastStartTimeTo}
          AND (
            COALESCE(lo.trace_id, '') > ${props.cursor.lastTraceId}
            OR (
              COALESCE(lo.trace_id, '') = ${props.cursor.lastTraceId}
              AND lo.id > ${props.cursor.lastId}
            )
          )
        )
      )
  ...
  const rows = await tracingPrisma.$queryRaw<PublicObservationRow[]>(Prisma.sql`
    ...
    FROM latest_observations lo
    ${cursorWhereSql}
    ORDER BY lo.start_time DESC, COALESCE(lo.trace_id, '') ASC, lo.id ASC
    LIMIT ${props.limit + 1}
  `);
